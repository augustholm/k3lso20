...
def solve_IK_test(coord, hip, leg, foot, left_side):

    x = coord[0]
    y = coord[1]
    z = coord[2]

    d1 = np.sqrt(x ** 2 + z ** 2 - hip ** 2)

    gamma = np.arcsin(x / d1)

    theta1_offset = np.pi / 2

    if left_side:
        hip = -hip
        theta1_offset = -np.pi / 2

    theta1 = -np.arctan(y / z) - np.arctan(d1 / hip) + theta1_offset

    theta2 = np.arccos((d1 ** 2 + leg ** 2 - foot ** 2) / (2 * d1 * leg)) - gamma
    theta3 = -np.pi + np.arccos((leg ** 2 + foot ** 2 - d1 ** 2) / (2 * leg * foot))
