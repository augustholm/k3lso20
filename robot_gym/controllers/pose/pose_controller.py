...
class PoseController(Controller):
    ...
    def get_action(self):
        ...
        front_right_angles_t = kinematics.solve_IK_test(t_front_right_coord, self._constants.hip, self._constants.leg, self._constants.foot, False)
        front_left_angles_t = kinematics.solve_IK_test(t_front_left_coord, self._constants.hip, self._constants.leg, self._constants.foot, True)
        rear_right_angles_t = kinematics.solve_IK_test(t_rear_right_coord, self._constants.hip, self._constants.leg, self._constants.foot, False)
        rear_left_angles_t = kinematics.solve_IK_test(t_rear_left_coord, self._constants.hip, self._constants.leg, self._constants.foot, True)
        
        
        signal = [
            front_right_angles_t[0], front_right_angles_t[1], front_right_angles_t[2],
            -front_left_angles_t[0], front_left_angles_t[1], -front_left_angles_t[2],
            rear_right_angles_t[0], -rear_right_angles_t[1], rear_right_angles_t[2],
            -rear_left_angles_t[0], -rear_left_angles_t[1], -rear_left_angles_t[2]
        ]
        '''
        signal = [
        ...
        if not np.array_equal(signal, self.last_signal):
            print(f"Front right difference: {front_right_angles-front_right_angles_t}")
            print(f"Front left difference: {front_left_angles-front_left_angles_t}")
            print(f"Rear left difference: {rear_left_angles-rear_left_angles_t}")
            print(f"Rear right difference: {rear_right_angles-rear_right_angles_t}\n")
