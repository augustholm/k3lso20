...
class Terrain:
    ...
    def generate_terrain(self, pybullet_client, height_perturbation_range=0.06):
        ...
        if self.terrain_type == 'plane':
            self.id = pybullet_client.loadURDF("plane.urdf")    #Kanske är här någonstans som man bör kasta in objektet?
            ...
            if self.terrain_id == 'objects':
                cubeid = pybullet_client.loadURDF("cube.urdf", [2, 0, 0], useFixedBase=1)
                #pybullet_client.createVisualShape(shapeType=pybullet_client.GEOM_BOX, visualFramePosition=[1, 0, 0])
                                #Vet inte vad det är som gör att createVisualShape krashar, den verkar inte gilla att
                                #filen heter box.urdf, använde cube.urdf men färgen smälter in lite med bakrunden
        else:
    ...
    def setup_ui_params(pybullet_client):
        ...
        objects = pybullet_client.addUserDebugParameter("Objects", 0, -1, 0)  #Ny tillagd, första argumentet bestämmer
                                                                              #vad som kommer att stå på knappen
    ...
    def parse_ui_input(self, ui, pybullet_client):
        ...
        if pybullet_client.readUserDebugParameter(ui["plane"]):
            terrain_type = "plane"  #Tror att "terrain_type" är den generella typen och "terrain_id" är den specifika
            terrain_id = None       #Eller så handlar bara "terrain_type" om hur den skapas och id är själva profilen
            reset_sim = True
        ...
        #     self.update_terrain()
             ...
             terrain_type = "png"       #Denna elseif sats var bortkommenterad och den tillåter oss att skapa terrängen "Maze"
             terrain_id = "maze"
             reset_sim = True
            ...
            terrain_id = "hills"
            reset_sim = True
        elif pybullet_client.readUserDebugParameter(ui["objects"]): #Nytillagd
            terrain_type = "plane"  #Tror att det är den här parametern som avgör vilken terräng som skapas
            terrain_id = "objects"   #Vissa terränger har uttryckligt en beskrivning som matchar UI frasen här.
            reset_sim = True
        else:
