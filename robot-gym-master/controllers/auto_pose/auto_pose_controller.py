...
from robot_gym.controllers.auto_pose import kinematics
from robot_gym.model.robots import simple_motor

import time

class AutoPoseController(Controller):

    MOTOR_CONTROL_MODE = simple_motor.MOTOR_CONTROL_POSITION

    def __init__(self, robot, get_time_since_reset):
        super(AutoPoseController, self).__init__(robot, get_time_since_reset)
    ...
    def generate_actions(self):
        initial_actions = []
        actions = []
        #actions.append([[0.2, 0, 0], [0, 0 ,0]])
        #actions.append([[0, 0.2, 0], [0, 0 ,0]])
        #actions.append([[-0.2, 0, 0], [0, 0 ,0]])
        #actions.append([[0, -0.2, 0], [0, 0 ,0]])

        '''TEST'''
        for i in np.arange(0, 0.2, 0.008): #Go center-right, SHOULD only be run first time
            initial_actions.append([[i, 0, 0], [0, 0 ,0]])

        for i in np.arange(0, 0.2, 0.008): #Go front-right
            actions.append([[0.2, i, 0], [0, 0 ,0]])

        for i in reversed(np.arange(-0.2, 0.2, 0.008)): #Go front-left
            actions.append([[i, 0.2, 0], [0, 0 ,0]])

        for i in reversed(np.arange(-0.2, 0.2, 0.008)): #Go rear-left
            actions.append([[-0.2, i, 0], [0, 0 ,0]]) 
        
        for i in np.arange(-0.2, 0.2, 0.008): #Go rear-right
            actions.append([[i, -0.2, 0], [0, 0 ,0]])

        for i in np.arange(-0.2, 0, 0.008): #Go center-right
            actions.append([[0.2, i, 0], [0, 0 ,0]])
        '''TEST END'''
    ...
    def get_action(self):
        # runs an action every 5 seconds
        print(time.time_ns() - self.last_action_time)
        if (time.time_ns() - self.last_action_time) > 100000:

            self.last_action_time = time.time_ns()

            if len(self.actions) == 0: self.generate_actions()

            if len(self.initial_actions):
                action = self.initial_actions.pop(0)
            else:
                action = self.actions[self.current_action]
                self.current_action = self.current_action + 1

            orientation = np.array(action[0])
            position = np.array(action[1])

            if self.current_action == self.num_actions:
                self.current_action = 0
            print(action[0])
            # foot positons relative to the robot
