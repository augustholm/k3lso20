...
class Robot:
    ...
    def ConvertPositionsToK3lso(self, positions):
        newPositions = []

        #Rätt från k3lso
        '''offsets = [[-0.00, True], [0.92, True], [-1.483529864, True],
                   [-0.00, True], [0.92, False], [-1.483529864, False],
                   [-0.00, False], [1.047197551, True], [-1.483529864, True],
                   [-0.00, False], [1.047197551, False], [-1.483529864, False]]'''

        #Test
        offsets = [[-0.00, False], [0.92, True], [-1.483529864, True],
                   [-0.00, False], [0.92, True], [-1.483529864, False],
                   [-0.00, False], [1.047197551, False], [-1.483529864, True],
                   [-0.00, False], [1.047197551, False], [-1.483529864, False]]
        ...
        for i, pos in enumerate(positions):
            ...
            pos = (pos + offsets[i][0]) / (2 * np.pi) # Här så skall den konvertera från radianer till "procent"?
    ...
    def ApplyAction(self, motor_commands, motor_control_mode):
        ...
        if not np.array_equal(motor_commands, self.old_motor_commands):
            print(motor_commands)
            print(self.ConvertPositionsToK3lso(motor_commands))
            print("\n\n")

        self.old_motor_commands = motor_commands
